...
`)}var $=!!window.CSS?.highlights&&typeof globalThis.Highlight<"u",S=class{constructor(e){this.body="";this.quotes=[];this.mode="write";this.seq=0;this.saveTimer=null;this.savedAt="";this.api=e.api,this.root=e.root,this.source=e.source,this.commitSha=e.commitSha,this.userName=e.userName,this.onClose=e.onClose}async open(){let e=await this.api.getResponse(this.source).catch(()=>null);e&&(this.body=e.body??"",this.quotes=e.quotes??[],this.savedAt=e.updated??""),this.build(),this.renderQuoteHighlights()}close(){if(this.flushSave(),$){let e=window.CSS.highlights;e.delete("penumbra-quote"),e.delete("penumbra-quote-active")}this.el?.remove(),j(),this.onClose()}build(){let e=document.createElement("div");e.className="pen-panel",e.setAttribute("data-pen-ui",""),e.innerHTML=`
      ...
      <div class="pen-preview" data-preview hidden></div>`,document.body.appendChild(e),this.el=e,Q(),this.ta=e.querySelector("[data-essay]"),this.ta.value=this.body,e.querySelector('[data-act="close"]').addEventListener("click",()=>this.close()),e.querySelector('[data-act="mode"]').addEventListener("click",()=>this.toggleMode()),e.querySelector('[data-act="submit"]').addEventListener("click",()=>this.submit()),this.ta.addEventListener("input",()=>{this.body=this.ta.value,this.renderQuoteHighlights(),this.scheduleSave()}),this.ta.addEventListener("paste",t=>this.onPaste(t)),this.ta.addEventListener("keyup",()=>this.amplifyAtCursor()),this.ta.addEventListener("click",()=>this.amplifyAtCursor())}appendQuote(e){let t=x(e,this.root);if(!t)return;let n=t.find(o=>o.type==="TextQuoteSelector")?.exact??"";n&&(this.quotes.some(o=>o.text===n)||this.quotes.push({id:`q${++this.seq}`,selector:t,text:n}),this.body=`${this.ta.value.replace(/\s+$/,"")}

> ${n.replace(/\n/g," ")}

`,this.ta.value=this.body,this.ta.selectionStart=this.ta.selectionEnd=this.ta.value.length,this.ta.focus(),this.renderQuoteHighlights(),this.scheduleSave())}onPaste(e){let t=e.clipboardData?.getData("text/plain")??"";setTimeout(()=>{this.body=this.ta.value,this.scheduleSave();let n=M(t,this.root);if(n){let o=n.find(s=>s.type==="TextQuoteSelector").exact;this.quotes.some(s=>s.text===o)||(this.quotes.push({id:`q${++this.seq}`,selector:n,text:o}),this.renderQuoteHighlights())}},0)}quoteRange(e){return L(e.selector,this.root)}liveQuotes(){let e=this.ta.value.replace(/\s+/g," ");return this.quotes.filter(t=>e.includes(t.text.replace(/\s+/g," ").trim()))}renderQuoteHighlights(){if(!$)return;let e=this.liveQuotes().map(n=>this.quoteRange(n)).filter(Boolean),t=window.CSS.highlights;e.length?t.set("penumbra-quote",new globalThis.Highlight(...e)):t.delete("penumbra-quote")}amplifyAtCursor(){let e=this.ta.value,t=this.ta.selectionStart,n=e.lastIndexOf(`
`,t-1)+1,o=e.indexOf(`
