"use strict";(()=>{var k="penumbra:token",f=class{constructor(e){this.base=e}get token(){return localStorage.getItem(k)}set token(e){e?localStorage.setItem(k,e):localStorage.removeItem(k)}headers(e=!1){let n={};return e&&(n["Content-Type"]="application/json"),this.token&&(n.Authorization=`Bearer ${this.token}`),n}captureTokenFromHash(){let e=/[#&]pen_token=([^&]+)/.exec(location.hash);return e?(this.token=decodeURIComponent(e[1]),history.replaceState(null,"",location.pathname+location.search),!0):!1}async me(){if(!this.token)return{user:null,isAuthor:!1};let e=await fetch(`${this.base}/me`,{headers:this.headers()});if(!e.ok)return{user:null,isAuthor:!1};let n=await e.json();return{user:n.user,isAuthor:!!n.isAuthor}}async list(e,n=[]){let t=new URLSearchParams({source:e});n.length&&t.set("include",n.join(","));let o=await fetch(`${this.base}/annotations?${t}`);return o.ok?(await o.json()).items:[]}async create(e,n,t={}){let o=await fetch(`${this.base}/annotations`,{method:"POST",headers:this.headers(!0),body:JSON.stringify({target:e,kind:t.kind??"comment",body:[{type:"TextualBody",value:n}],docVersion:t.docVersion})});if(!o.ok)throw new Error((await o.json().catch(()=>({}))).error??`create failed (${o.status})`);return o.json()}async reply(e,n){let t=e.split("/annotations/")[1],o=await fetch(`${this.base}/annotations/${t}/replies`,{method:"POST",headers:this.headers(!0),body:JSON.stringify({text:n})});if(!o.ok)throw new Error((await o.json().catch(()=>({}))).error??`reply failed (${o.status})`);return o.json()}async patch(e,n){let t=e.split("/annotations/")[1],o=await fetch(`${this.base}/annotations/${t}`,{method:"PATCH",headers:this.headers(!0),body:JSON.stringify(n)});if(!o.ok)throw new Error(`patch failed (${o.status})`);return o.json()}async remove(e){let n=e.split("/annotations/")[1];await fetch(`${this.base}/annotations/${n}`,{method:"DELETE",headers:this.headers()})}async getResponse(e){let n=await fetch(`${this.base}/responses?source=${encodeURIComponent(e)}`,{headers:this.headers()});return n.ok?(await n.json()).response:null}async saveResponse(e,n,t,o){let i=await fetch(`${this.base}/responses`,{method:"POST",headers:this.headers(!0),body:JSON.stringify({source:e,body:n,quotes:t,sourceSha:o})});if(!i.ok)throw new Error((await i.json().catch(()=>({}))).error??`save failed (${i.status})`);return i.json()}async getAllResponses(e){let n=await fetch(`${this.base}/responses/all?source=${encodeURIComponent(e)}`,{headers:this.headers()});return n.ok?(await n.json()).responses:[]}async submitResponse(e){let n=await fetch(`${this.base}/responses/submit`,{method:"POST",headers:this.headers(!0),body:JSON.stringify({source:e})}),t=await n.json().catch(()=>({}));if(!n.ok)throw new Error(t.error??`submit failed (${n.status})`);return t}async uploadImage(e){let n={"Content-Type":e.type||"application/octet-stream"};this.token&&(n.Authorization=`Bearer ${this.token}`);let t=await fetch(`${this.base}/upload`,{method:"POST",headers:n,body:e});if(!t.ok)throw new Error(`upload failed (${t.status})`);return(await t.json()).url}loginUrl(e){return`${this.base}/auth/${e}/start?return=${encodeURIComponent(location.href)}`}async emailLogin(e){return(await fetch(`${this.base}/auth/email/start`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,return:location.href})})).json()}async logout(){await fetch(`${this.base}/auth/logout`,{method:"POST",headers:this.headers()}).catch(()=>{}),this.token=null}};function E(s){let e=document.createTreeWalker(s,NodeFilter.SHOW_TEXT,{acceptNode(i){let r=i.parentElement;return r&&r.closest("[data-pen-ui]")?NodeFilter.FILTER_REJECT:NodeFilter.FILTER_ACCEPT}}),n=[],t="",o;for(;o=e.nextNode();){let i=o,r=t.length;t+=i.data,n.push({node:i,start:r,end:t.length})}return{text:t,nodes:n}}function M(s,e){for(let t of s.nodes)if(e>=t.start&&e<=t.end)return{node:t.node,offset:e-t.start};let n=s.nodes[s.nodes.length-1];return n?{node:n.node,offset:n.node.data.length}:null}function $(s,e,n){if(e.nodeType===Node.TEXT_NODE){let i=s.nodes.find(r=>r.node===e);return i?i.start+n:null}let t=e.childNodes[n]??e.childNodes[e.childNodes.length-1];if(!t)return null;let o=s.nodes.find(i=>i.node===t||t.contains(i.node));return o?o.start:null}function P(s,e){let n=E(e),t=$(n,s.startContainer,s.startOffset),o=$(n,s.endContainer,s.endOffset);if(t==null||o==null||o<=t)return null;let i=n.text.slice(t,o),r=n.text.slice(Math.max(0,t-32),t),a=n.text.slice(o,o+32);return[{type:"TextQuoteSelector",exact:i,prefix:r,suffix:a},{type:"TextPositionSelector",start:t,end:o}]}function v(s,e){let n=s.find(p=>p.type==="TextQuoteSelector");if(!n?.exact)return null;let t=E(e),o=_(t.text,n);if(o<0)return null;let i=M(t,o),r=M(t,o+n.exact.length);if(!i||!r)return null;let a=document.createRange();return a.setStart(i.node,i.offset),a.setEnd(r.node,r.offset),a}function S(s,e){let n=s.trim();if(n.length<8)return null;let t=E(e),o=t.text.indexOf(n);return o<0?null:[{type:"TextQuoteSelector",exact:n,prefix:t.text.slice(Math.max(0,o-32),o),suffix:t.text.slice(o+n.length,o+n.length+32)},{type:"TextPositionSelector",start:o,end:o+n.length}]}function _(s,e){let n=[],t=s.indexOf(e.exact);for(;t>=0;)n.push(t),t=s.indexOf(e.exact,t+1);if(n.length===0)return-1;if(n.length===1)return n[0];let o=n[0],i=-1;for(let r of n){let a=0;if(e.prefix){let p=s.slice(Math.max(0,r-e.prefix.length),r);a+=X(p,e.prefix)}if(e.suffix){let p=s.slice(r+e.exact.length,r+e.exact.length+e.suffix.length);a+=D(p,e.suffix)}a>i&&(i=a,o=r)}return o}var D=(s,e)=>{let n=0;for(;n<s.length&&n<e.length&&s[n]===e[n];)n++;return n},X=(s,e)=>{let n=0;for(;n<s.length&&n<e.length&&s[s.length-1-n]===e[e.length-1-n];)n++;return n};var H=s=>s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");function L(s){let e=[],n=[];for(let t of(s??"").split(`
`))/^\s*>/.test(t)?n.push(t.replace(/^\s*>\s?/,"")):n.length&&(e.push(n.join(" ").trim()),n=[]);return n.length&&e.push(n.join(" ").trim()),e.filter(t=>t.length>=6)}var T=s=>/^\s*>/.test(s??"");function A(s){let e=(s??"").replace(/\r\n/g,`
`).split(`
`),n=0,t=[];for(;n<e.length&&!T(e[n]);)t.push(e[n]),n++;let o=[];for(;n<e.length;){let i=[];for(;n<e.length&&T(e[n]);)i.push(e[n].replace(/^\s*>\s?/,"")),n++;let r=[];for(;n<e.length&&!T(e[n]);)r.push(e[n]),n++;for(;r.length&&r[0].trim()==="";)r.shift();r.length&&r[r.length-1].trim()===""&&r.pop(),o.push({quotes:[i.join(" ").trim()],note:r.join(`
`)})}return{preamble:t.join(`
`).trim(),blocks:o}}function C(s,e){let n=[];s.trim()&&n.push(s.trim());for(let t of e){let o=t.note.replace(/^\n+/,"");if(!t.quotes.join("").trim()&&!o.trim())continue;let i=t.quotes.map(r=>`> ${r.replace(/\n/g," ")}`).join(`
>
`);n.push(o?`${i}
...
`)+`
`}function B(s){let e=s??"",n=[],t=0,o=Intl.Segmenter;if(typeof o=="function")for(let{segment:i}of new o("en",{granularity:"grapheme"}).segment(e)){if(/^\s+$/.test(i)){t+=i.length;continue}if(/\p{Extended_Pictographic}/u.test(i)){n.push(i),t+=i.length;continue}break}else{let i=/^((?:\p{Extended_Pictographic}(?:[️‍\u{1F3FB}-\u{1F3FF}]|\p{Extended_Pictographic})*|\s)+)/u.exec(e);if(i){t=i[0].length;for(let r of[...i[0]])/\p{Extended_Pictographic}/u.test(r)&&n.push(r)}}return{emojis:n,text:e.slice(t)}}function b(s){return H(s).replace(/!\[([^\]]*)\]\(([^)\s]+)\)/g,'<img alt="$1" src="$2">').replace(/\[([^\]]+)\]\(([^)\s]+)\)/g,'<a href="$2" target="_blank" rel="noopener">$1</a>').replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/(^|[^*])\*([^*\n]+)\*/g,"$1<em>$2</em>")}function u(s){let e=(s??"").replace(/\r\n/g,`
`).split(`
`),n=[],t=0;for(;t<e.length;){let o=e[t];if(/^```/.test(o)){let r=[];for(t++;t<e.length&&!/^```/.test(e[t]);)r.push(e[t++]);t++,n.push(`<pre><code>${H(r.join(`
`))}</code></pre>`);continue}if(/^\s*$/.test(o)){t++;continue}if(/^#{1,6}\s/.test(o)){let r=/^(#{1,6})\s+(.*)$/.exec(o);n.push(`<h${r[1].length}>${b(r[2])}</h${r[1].length}>`),t++;continue}if(/^\s*([-*_])\1{2,}\s*$/.test(o)){n.push("<hr>"),t++;continue}if(/^\s*>/.test(o)){let r=[];for(;t<e.length&&/^\s*>/.test(e[t]);)r.push(e[t++].replace(/^\s*>\s?/,""));n.push(`<blockquote>${u(r.join(`
`))}</blockquote>`);continue}if(/^\s*[-*+]\s/.test(o)){let r=[];for(;t<e.length&&/^\s*[-*+]\s/.test(e[t]);)r.push(`<li>${b(e[t++].replace(/^\s*[-*+]\s+/,""))}</li>`);n.push(`<ul>${r.join("")}</ul>`);continue}if(/^\s*\d+\.\s/.test(o)){let r=[];for(;t<e.length&&/^\s*\d+\.\s/.test(e[t]);)r.push(`<li>${b(e[t++].replace(/^\s*\d+\.\s+/,""))}</li>`);n.push(`<ol>${r.join("")}</ol>`);continue}let i=[];for(;t<e.length&&!/^\s*$/.test(e[t])&&!/^(#{1,6}\s|```|\s*>|\s*[-*+]\s|\s*\d+\.\s)/.test(e[t]);)i.push(e[t++]);n.push(`<p>${b(i.join(" "))}</p>`)}return n.join(`
`)}var x=class{constructor(e){this.reviews=[];this.api=e.api,this.root=e.root,this.source=e.source,this.onClose=e.onClose}async open(){this.reviews=await this.api.getAllResponses(this.source).catch(()=>[]);let e=document.createElement("div");e.className="pen-panel",e.setAttribute("data-pen-ui",""),e.innerHTML=`
      ...
      <div class="pen-reviews" data-list></div>`;let n=e.querySelector("[data-list]");n.innerHTML=this.reviews.length?this.reviews.map((t,o)=>{let i=(t.quotes??[]).filter(r=>!r.dismissed);return`<div class="pen-review">
            <div class="pen-review-head"><span class="pen-name">${N(t.creator?.name??"reader")}</span>
              <span class="pen-savestate">${U(t.updated)} \xB7 ${t.status}</span></div>
            <div class="pen-md">${u(t.body||"_(no writing yet)_")}</div>
            ${i.length?`<div class="pen-review-quotes">${i.map((r,a)=>`<a class="pen-qchip" data-ri="${o}" data-qi="${a}">\u201C${N(Y(r.text))}\u201D</a>`).join("")}</div>`:""}
          </div>`}).join(""):'<p style="padding:14px;color:var(--pen-muted)">No reviews yet.</p>',e.querySelector('[data-act="close"]').addEventListener("click",()=>this.close()),n.querySelectorAll(".pen-qchip").forEach(t=>t.addEventListener("click",()=>this.focusQuote(+t.dataset.ri,+t.dataset.qi))),document.body.appendChild(e),this.el=e,document.body.classList.add("pen-panel-open")}focusQuote(e,n){let t=this.reviews[e]?.quotes?.[n];if(!t)return;let o=v(t.selector,this.root);if(!o)return;let i=window.CSS?.highlights;i&&i.set("penumbra-quote-active",new globalThis.Highlight(o));let r=o.getBoundingClientRect();window.scrollTo({top:window.scrollY+r.top-120,behavior:"smooth"})}close(){let e=window.CSS?.highlights;e&&e.delete("penumbra-quote-active"),this.el?.remove(),document.body.classList.remove("pen-panel-open"),this.onClose()}};function N(s){return String(s).replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}var U=s=>{try{return new Date(s).toLocaleDateString()}catch{return""}},Y=(s,e=60)=>s.length>e?s.slice(0,e)+"\u2026":s;var z=`
...
`;var w=null;function O(){return window.__PenumbraResponsePanel?Promise.resolve():w||(w=new Promise((s,e)=>{let n=document.querySelector('script[src*="penumbra.js"]'),t=n?n.src.replace(/penumbra\.js(\?.*)?$/,"penumbra-editor.js"):"/static/penumbra-editor.js",o=document.createElement("script");o.src=t,o.onload=()=>s(),o.onerror=()=>e(new Error("editor failed to load")),document.head.appendChild(o)}),w)}async function J(){return await O(),window.__PenumbraResponsePanel}async function Q(){return await O(),window.__PenumbraMiniEditor}var W=typeof globalThis.Highlight<"u"&&!!window.CSS?.highlights,q=10,R=6,V=["\u{1F44D}","\u2764\uFE0F","\u{1F525}","\u{1F604}","\u{1F914}","\u{1F3AF}"],K=[["\u{1F44D}","thumbs up like yes good approve"],["\u{1F44E}","thumbs down dislike no bad"],["\u2764\uFE0F","heart love red"],["\u{1F525}","fire lit hot flame"],["\u{1F4AF}","hundred 100 perfect score"],["\u{1F389}","party tada celebrate congrats"],["\u{1F680}","rocket launch fast ship"],["\u{1F4A1}","idea bulb light"],["\u2705","check done yes correct"],["\u274C","cross no wrong x"],["\u2B50","star favorite"],["\u{1F64F}","pray thanks please hands"],["\u{1F44F}","clap applause bravo"],["\u{1F440}","eyes look watching"],["\u{1F914}","think thinking hmm"],["\u{1F602}","laugh lol joy cry"],["\u{1F60D}","love eyes heart"],["\u{1F62E}","wow surprised"],["\u{1F622}","sad cry tear"],["\u{1F621}","angry mad rage"],["\u{1F605}","sweat nervous laugh"],["\u{1F60E}","cool sunglasses"],["\u{1F92F}","mind blown exploding head"],["\u{1F64C}","raised hands celebrate yay"],["\u{1F4AA}","muscle strong flex"],["\u{1F91D}","handshake deal agree"],["\u{1F9E0}","brain smart mind"],["\u{1F4CC}","pin important"],["\u26A1","lightning fast bolt energy"],["\u{1F31F}","glowing star"],["\u2728","sparkles shiny magic"],["\u{1F480}","skull dead lol"],["\u{1F979}","holding back tears touched"],["\u{1FAE1}","salute respect"],["\u{1F937}","shrug dunno whatever"],["\u{1FAE0}","melting embarrassed"],["\u{1F4DD}","memo note write"],["\u{1F516}","bookmark save tag"],["\u2753","question"],["\u2757","exclamation important"],["\u{1F600}","grin happy smile"],["\u{1F642}","slight smile"],["\u{1F609}","wink"],["\u{1F928}","raised eyebrow suspicious doubt"],["\u{1F644}","eye roll annoyed"],["\u{1F634}","sleep tired bored"],["\u{1F973}","party face celebrate"],["\u{1F631}","scream shock fear"],["\u{1F913}","nerd glasses geek"],["\u{1FAF6}","heart hands love"],["\u{1F44C}","ok perfect nice"],["\u{1F90C}","chefs kiss pinch"],["\u270C\uFE0F","peace victory"],["\u{1F91E}","fingers crossed luck hope"],["\u{1F44B}","wave hi hello bye"],["\u{1F3AF}","target bullseye goal"],["\u{1F410}","goat greatest"],["\u{1F4B8}","money cash spend"],["\u{1F4C8}","chart up growth"],["\u{1F4C9}","chart down decline"]],G='<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M8 6V4h8v2M6 6l1 14h10l1-14"/></svg>',y=class{constructor(e){this.user=null;this.isAuthor=!1;this.preamble="";this.blocks=[];this.highlightsOn=!0;this.focused=null;this.hovered=null;this.hoverRaf=!1;this.relayoutQueued=!1;this.railEntries=[];this.repoQueued=!1;this.railLeft=0;this.quietTimer=null;this.composeNew=!1;this.blockById=e=>this.blocks.find(n=>n.id===e);this.docY=e=>e.getBoundingClientRect().top+window.scrollY;this.cards=()=>this.blocks.filter(e=>e.text.trim()&&e.ranges.length);this.emojiBlocks=()=>this.blocks.filter(e=>e.emojis.length&&e.ranges.length);this.cfg=e,this.api=new f(e.api),this.root=this.resolveRoot(),this.source=this.computeSource(),this.commitSha=e.commitSha??null}resolveRoot(){return(this.cfg.root?document.querySelector(this.cfg.root):null)??document.body}computeSource(){if(this.cfg.source)return this.cfg.source;if(this.cfg.sourceBase){let e=location.pathname.replace(/\/index\.html?$/i,"/").replace(/\.html?$/i,"").replace(/\/$/,"");return this.cfg.sourceBase.replace(/\/$/,"")+e}return location.href}async init(){this.styleEl=document.createElement("style"),this.styleEl.setAttribute("data-pen",""),this.styleEl.textContent=z,document.head.appendChild(this.styleEl),this.layer=document.createElement("div"),this.layer.setAttribute("data-pen-ui",""),this.layer.style.cssText="position:absolute;top:0;left:0;width:0;height:0;",document.body.appendChild(this.layer),this.api.captureTokenFromHash();let e=await this.api.me();this.user=e.user,this.isAuthor=e.isAuthor,this.renderToolbar(),this.renderLogin(),await this.loadDoc(),document.addEventListener("mouseup",n=>{n.target.closest("[data-pen-ui]")||setTimeout(()=>this.onSelection(),0)}),document.addEventListener("mousedown",n=>this.onDocMouseDown(n)),document.addEventListener("click",n=>this.onDocClick(n)),document.addEventListener("mousemove",n=>this.onMouseMove(n),{passive:!0}),document.addEventListener("keydown",n=>{n.key==="Escape"&&(this.compose?this.dismissCompose():this.focused&&(this.focused=null,this.renderAll()))}),window.addEventListener("resize",()=>this.queueRelayout(),{passive:!0})}async reload(){this.dismissCompose(),this.removeQuoteBtn(),this.focused=this.hovered=null,this.styleEl.isConnected||document.head.appendChild(this.styleEl),this.layer.isConnected||document.body.appendChild(this.layer),this.toolbar?.isConnected||this.renderToolbar(),this.loginEl?.isConnected||this.renderLogin(),this.root=this.resolveRoot(),this.source=this.computeSource(),await this.loadDoc()}async loadDoc(){let e="";this.user&&(e=(await this.api.getResponse(this.source).catch(()=>null))?.body??""),this.parse(e),this.renderAll()}parse(e){let{preamble:n,blocks:t}=A(e);this.preamble=n,this.blocks=t.map((o,i)=>{let{emojis:r,text:a}=B(o.note);return{id:`b${i}`,quotes:o.quotes,note:o.note,emojis:r,text:a,ranges:o.quotes.map(p=>v([{type:"TextQuoteSelector",exact:p}],this.root)).filter(Boolean)}})}async saveDoc(){let e=C(this.preamble,this.blocks.map(t=>({quotes:t.quotes,note:t.note}))),n=L(e).map((t,o)=>({id:`q${o}`,text:t,selector:S(t,this.root)??[{type:"TextQuoteSelector",exact:t}]}));try{await this.api.saveResponse(this.source,e,n,this.commitSha)}catch(t){alert("Could not save: "+t.message);return}this.focused=null,this.parse(e),this.renderAll()}async serializeAndSave(){let e=C(this.preamble,this.blocks.map(t=>({quotes:t.quotes,note:t.note}))),n=L(e).map((t,o)=>({id:`q${o}`,text:t,selector:S(t,this.root)??[{type:"TextQuoteSelector",exact:t}]}));try{return await this.api.saveResponse(this.source,e,n,this.commitSha),!0}catch{return!1}}saveQuiet(e){let n=t=>{let o=e?.querySelector("[data-cardsave]");o&&(o.textContent=t)};n("saving\u2026"),clearTimeout(this.quietTimer),this.quietTimer=setTimeout(async()=>n(await this.serializeAndSave()?"saved":"save failed"),600)}flushQuiet(){clearTimeout(this.quietTimer),this.serializeAndSave()}composeNote(e,n){let t=e.join(""),o=n.replace(/^\s+/,"");return o.trim()?t?`${t} ${o}`:o:t}renderAll(){this.renderHighlights(),this.layoutRightRail(),this.layoutLeftRail()}queueRelayout(){this.relayoutQueued||(this.relayoutQueued=!0,requestAnimationFrame(()=>{this.relayoutQueued=!1,this.renderAll()}))}renderHighlights(){if(!W)return;let e=window.CSS.highlights,n=globalThis.Highlight;if(this.responsePanel)return;if(!this.highlightsOn){e.delete("penumbra-quote"),e.delete("penumbra-quote-active"),e.delete("penumbra-draft");return}let t=this.blocks.flatMap(i=>i.ranges);t.length?e.set("penumbra-quote",new n(...t)):e.delete("penumbra-quote"),this.composeCtx?.range?e.set("penumbra-draft",new n(this.composeCtx.range)):e.delete("penumbra-draft");let o=this.blockById(this.hovered??this.focused)?.ranges[0];o?e.set("penumbra-quote-active",new n(o)):e.delete("penumbra-quote-active")}layoutRightRail(){if(this.railRO?.disconnect(),this.destroyCardEditor(),this.layer.querySelectorAll(".pen-card.rail").forEach(o=>o.remove()),this.railEntries=[],!this.highlightsOn||this.responsePanel)return;let e=this.root.getBoundingClientRect();if(window.innerWidth-e.right<300)return;this.railLeft=window.scrollX+e.right+24;let n=this.cards().sort((o,i)=>this.docY(o.ranges[0])-this.docY(i.ranges[0]));if(!n.length)return;for(let o of n){let i=this.buildCard(o,this.focused===o.id);i.style.left=`${this.railLeft}px`,i.style.top="-9999px",this.layer.appendChild(i),this.railEntries.push({el:i,blk:o})}this.railRO=new ResizeObserver(()=>this.queueReposition()),this.railEntries.forEach(o=>this.railRO.observe(o.el)),this.repositionRail();let t=this.railEntries.find(o=>o.blk.id===this.focused);t&&this.mountCardEditor(t.el,t.blk)}async mountCardEditor(e,n){let t=e.querySelector("[data-note-editor]");if(!t)return;let o;try{o=await Q()}catch{return}this.focused!==n.id||!t.isConnected||(this.destroyCardEditor(),t.textContent="",this.cardEditor=o(t,n.text,{onChange:i=>{n.text=i,n.note=this.composeNote(n.emojis,i),this.queueReposition(),this.saveQuiet(e)},uploadImage:i=>this.api.uploadImage(i)}),this.cardEditor.focus())}destroyCardEditor(){this.cardEditor&&(this.flushQuiet(),this.cardEditor.destroy(),this.cardEditor=void 0)}queueReposition(){this.repoQueued||(this.repoQueued=!0,requestAnimationFrame(()=>{this.repoQueued=!1,this.repositionRail()}))}repositionRail(){let e=this.railEntries;if(!e.length)return;let n=e.map(r=>r.el.offsetHeight),t=e.map(r=>r.blk.ranges[0]?this.docY(r.blk.ranges[0]):0),o=t.slice(),i=e.findIndex(r=>r.blk.id===this.focused);if(i>=0){o[i]=t[i];for(let r=i+1;r<e.length;r++)o[r]=Math.max(t[r],o[r-1]+n[r-1]+q);for(let r=i-1;r>=0;r--)o[r]=Math.min(t[r],o[r+1]-n[r]-q)}else for(let r=1;r<e.length;r++)o[r]=Math.max(t[r],o[r-1]+n[r-1]+q);e.forEach((r,a)=>r.el.style.top=`${Math.max(0,o[a])}px`)}layoutLeftRail(){if(this.layer.querySelectorAll(".pen-emote-stack").forEach(o=>o.remove()),!this.highlightsOn||this.responsePanel)return;let e=this.root.getBoundingClientRect(),n=window.scrollX+Math.max(6,e.left-40),t=0;for(let o of this.emojiBlocks().sort((i,r)=>this.docY(i.ranges[0])-this.docY(r.ranges[0]))){let i=document.createElement("div");i.className="pen-emote-stack",i.setAttribute("data-pen-ui",""),i.dataset.blockId=o.id,i.innerHTML=o.emojis.map(a=>`<span class="pen-emote">${g(a)}</span>`).join(""),i.title="Edit reaction",i.addEventListener("mouseenter",()=>this.setHovered(o.id)),i.addEventListener("mouseleave",()=>this.setHovered(null)),i.addEventListener("click",()=>this.editBlock(o)),this.layer.appendChild(i);let r=Math.max(this.docY(o.ranges[0]),t+6);i.style.left=`${n}px`,i.style.top=`${r}px`,t=r+i.offsetHeight}}buildCard(e,n){let t=document.createElement("div");t.className=`pen-card rail ${n?"focused":"compact"}`,t.setAttribute("data-pen-ui",""),t.dataset.blockId=e.id;let o=e.quotes.map(i=>`<div class="pen-quote">${g(i)}</div>`).join("");if(n){t.innerHTML=`${o}
        <div class="pen-note-editor" data-note-editor><div class="pen-md">${u(e.text)}</div></div>
        ...
        <div class="pen-trashbox">
          <button class="pen-trash" data-act="del-init" title="Delete comment">${G}</button>
          ...
          </div></div>`,t.querySelector("[data-emojislot]").appendChild(this.buildEmojiPanel(()=>e.emojis,p=>this.toggleCardEmoji(e,p,t)));let r=t.querySelector("[data-confirm]"),a=t.querySelector('[data-act="del-init"]');a.addEventListener("click",()=>{a.style.display="none",r.hidden=!1}),t.querySelector('[data-act="del-no"]').addEventListener("click",()=>{r.hidden=!0,a.style.display=""}),t.querySelector('[data-act="del-yes"]').addEventListener("click",()=>{e.emojis.length?(e.text="",e.note=this.composeNote(e.emojis,"")):this.blocks=this.blocks.filter(p=>p.id!==e.id),this.saveDoc()})}else{let i=e.text.trim()?`<div class="pen-md">${u(e.text)}</div>`:'<div class="pen-md pen-muted">Add a comment\u2026</div>';t.innerHTML=`${o}<div class="pen-thread">${i}</div>`,t.addEventListener("click",()=>this.focus(e.id))}return t.addEventListener("mouseenter",()=>this.setHovered(e.id)),t.addEventListener("mouseleave",()=>this.setHovered(null)),t}focus(e){this.focused=e,this.renderAll()}toggleCardEmoji(e,n,t){this.cardEditor&&(e.text=this.cardEditor.getMarkdown());let o=e.emojis.indexOf(n);o>=0?e.emojis.splice(o,1):e.emojis.length<R&&e.emojis.push(n),e.note=this.composeNote(e.emojis,e.text),this.layoutLeftRail(),this.queueReposition(),this.saveQuiet(t)}setHovered(e){this.hovered!==e&&(this.hovered=e,this.layer.querySelectorAll("[data-block-id]").forEach(n=>n.classList.toggle("pen-emph",n.dataset.blockId===e)),this.renderHighlights())}onMouseMove(e){this.hoverRaf||this.responsePanel||(this.hoverRaf=!0,requestAnimationFrame(()=>{if(this.hoverRaf=!1,e.target?.closest?.("[data-pen-ui]"))return;let n=null;for(let t of this.blocks)if(t.ranges.some(o=>this.hitsRange(e,o))){n=t.id;break}this.setHovered(n)}))}onSelection(){let e=window.getSelection();if(!e||e.isCollapsed||e.rangeCount===0||!e.toString().trim()){this.removeQuoteBtn();return}let n=e.getRangeAt(0);if(this.root.contains(n.commonAncestorContainer)){if(this.responsePanel){this.showQuoteButton(n.cloneRange());return}this.openCompose(n.cloneRange())}}showQuoteButton(e){this.removeQuoteBtn();let n=e.getBoundingClientRect(),t=document.createElement("button");t.className="pen-addbtn",t.setAttribute("data-pen-ui",""),t.textContent="Quote",t.style.left=`${window.scrollX+n.left+n.width/2}px`,t.style.top=`${window.scrollY+n.top}px`,t.onmousedown=o=>{o.preventDefault(),o.stopPropagation()},t.onclick=()=>{this.responsePanel?.appendQuote(e),window.getSelection()?.removeAllRanges(),this.removeQuoteBtn()},this.layer.appendChild(t),this.quoteBtn=t}removeQuoteBtn(){this.quoteBtn?.remove(),this.quoteBtn=void 0}async openCompose(e,n){if(!this.user){e&&this.promptSignIn(e);return}let t="";if(n?t=n.quotes[0]??"":e&&(t=P(e,this.root)?.find(c=>c.type==="TextQuoteSelector")?.exact??""),!t)return;let o=e??n?.ranges[0]??null;if(!o)return;this.dismissCompose();let i=n??{id:`b${this.blocks.length}`,quotes:[t],note:"",emojis:[],text:"",ranges:[e.cloneRange()]},r=o.getBoundingClientRect(),a=document.createElement("div");a.className="pen-compose",a.setAttribute("data-pen-ui",""),a.style.left=`${Math.min(window.scrollX+r.left,window.scrollX+window.innerWidth-372)}px`,a.style.top=`${window.scrollY+r.bottom+8}px`,a.innerHTML='<div class="pen-note-editor" data-note-editor></div><div data-emojislot></div>',a.querySelector("[data-emojislot]").appendChild(this.buildEmojiPanel(()=>i.emojis,c=>this.toggleComposeEmoji(c))),this.layer.appendChild(a),this.compose=a,this.composeBlock=i,this.composeNew=!n,e&&(this.composeCtx={range:e,quote:t},this.renderHighlights());let p;try{p=await Q()}catch{return}if(this.compose!==a)return;let m=a.querySelector("[data-note-editor]");this.composeEditor=p(m,i.text,{onChange:c=>{i.text=c,i.note=this.composeNote(i.emojis,c),this.graftCompose(),this.saveQuiet()},uploadImage:c=>this.api.uploadImage(c)}),this.composeEditor.focus()}editBlock(e){this.openCompose(null,e)}graftCompose(){let e=this.composeBlock;if(!e||!this.composeNew)return;let n=this.blocks.includes(e);e.note.trim()&&!n?this.blocks.push(e):!e.note.trim()&&n&&(this.blocks=this.blocks.filter(t=>t!==e))}toggleComposeEmoji(e){let n=this.composeBlock;if(!n)return;this.composeEditor&&(n.text=this.composeEditor.getMarkdown());let t=n.emojis.indexOf(e);t>=0?n.emojis.splice(t,1):n.emojis.length<R&&n.emojis.push(e),n.note=this.composeNote(n.emojis,n.text),this.graftCompose(),this.layoutLeftRail(),this.saveQuiet()}promptSignIn(e){this.dismissCompose();let n=e.getBoundingClientRect(),t=document.createElement("div");t.className="pen-compose",t.setAttribute("data-pen-ui",""),t.style.left=`${Math.min(window.scrollX+n.left,window.scrollX+window.innerWidth-372)}px`,t.style.top=`${window.scrollY+n.bottom+8}px`,t.innerHTML=`<div class="pen-title" style="margin-bottom:8px">Sign in to comment on this.</div>
      <div class="pen-row"><span></span><button class="pen-btn" data-act="signin">Sign in</button></div>`,t.querySelector('[data-act="signin"]').addEventListener("click",()=>{this.dismissCompose(),this.flashLogin()}),this.layer.appendChild(t),this.compose=t}buildEmojiPanel(e,n){let t=document.createElement("div");t.className="pen-emojipanel",t.innerHTML=`
      ...
      </div>`;let o=t.querySelector("[data-bar]"),i=t.querySelector("[data-grid]"),r=t.querySelector("[data-more]"),a=t.querySelector(".pen-emoji-search"),p=(d,h)=>`<button class="${h.includes(d)?"selected":""}" data-e="${g(d)}">${g(d)}</button>`,m=()=>{let d=e(),h=a.value.trim().toLowerCase();i.innerHTML=K.filter(([l,F])=>!h||F.includes(h)||l===a.value).map(([l])=>p(l,d)).join(""),i.querySelectorAll("[data-e]").forEach(l=>l.addEventListener("click",()=>j(l.dataset.e)))},c=()=>{let d=e(),h=V.filter(l=>!d.includes(l)).slice(0,Math.max(0,R-d.length));o.innerHTML=d.map(l=>p(l,d)).join("")+h.map(l=>p(l,d)).join("")+'<button class="pen-emoji-more" data-act="more" title="More emoji">\uFF0B</button>',o.querySelectorAll("[data-e]").forEach(l=>l.addEventListener("click",()=>j(l.dataset.e))),o.querySelector('[data-act="more"]').addEventListener("click",()=>{r.hidden=!r.hidden,r.hidden||(m(),a.focus())})},j=d=>{n(d),c(),r.hidden||m()};return a.addEventListener("input",m),c(),t}teardownCompose(){clearTimeout(this.quietTimer),this.composeEditor?.destroy(),this.composeEditor=void 0,this.composeBlock=void 0,this.composeNew=!1,this.compose?.remove(),this.compose=void 0,this.composeCtx=void 0}dismissCompose(){if(!this.compose){this.renderHighlights();return}let e=this.composeBlock;e&&this.composeEditor&&(e.text=this.composeEditor.getMarkdown(),e.note=this.composeNote(e.emojis,e.text));let n=!!e&&(this.blocks.includes(e)||this.composeNew&&!!e.note.trim());e&&this.composeNew&&e.note.trim()&&!this.blocks.includes(e)&&this.blocks.push(e),e&&!e.note.trim()&&(this.blocks=this.blocks.filter(t=>t!==e)),this.teardownCompose(),n?this.saveDoc():this.renderAll()}onDocMouseDown(e){e.target.closest("[data-pen-ui]")||(this.dismissCompose(),this.removeQuoteBtn())}onDocClick(e){if(!e.target.closest("[data-pen-ui]")&&!window.getSelection()?.toString().trim()){for(let n of this.blocks)if(n.ranges.some(t=>this.hitsRange(e,t)))return n.text.trim()?this.focus(n.id):this.editBlock(n);this.focused&&(this.focused=null,this.renderAll())}}hitsRange(e,n){for(let t of n.getClientRects())if(e.clientX>=t.left&&e.clientX<=t.right&&e.clientY>=t.top&&e.clientY<=t.bottom)return!0;return!1}renderToolbar(){this.toolbar?.remove();let e=document.createElement("div");e.className="pen-toolbar",e.setAttribute("data-pen-ui",""),e.innerHTML=`
      ...
      ${this.isAuthor?`<button class="pen-tbtn" data-act="reviews" title="See everyone's responses">\u{1F441} Reviews</button>`:""}`,e.querySelector('[data-act="toggle"]').addEventListener("click",()=>{this.highlightsOn=!this.highlightsOn,e.querySelector('[data-act="toggle"]').classList.toggle("active",this.highlightsOn),this.renderAll()}),e.querySelector('[data-act="response"]').addEventListener("click",()=>this.toggleResponse()),e.querySelector('[data-act="reviews"]')?.addEventListener("click",()=>this.toggleReviews()),document.body.appendChild(e),this.toolbar=e}toggleReviews(){if(this.reviewsPanel){this.reviewsPanel.close();return}this.reviewsPanel=new x({api:this.api,root:this.root,source:this.source,onClose:()=>{this.reviewsPanel=void 0}}),this.reviewsPanel.open()}async toggleResponse(){if(this.responsePanel){this.responsePanel.close();return}if(!this.user)return this.flashLogin();let e;this.toolbar?.querySelector('[data-act="response"]')?.classList.add("active");try{e=await J()}catch{this.toolbar?.querySelector('[data-act="response"]')?.classList.remove("active"),alert("Could not load the editor.");return}this.responsePanel||(this.destroyCardEditor(),this.layer.querySelectorAll(".pen-card.rail, .pen-emote-stack").forEach(n=>n.remove()),this.responsePanel=new e({api:this.api,root:this.root,source:this.source,commitSha:this.commitSha,userName:this.user.name??"you",onClose:()=>{this.responsePanel=void 0,this.removeQuoteBtn(),this.toolbar?.querySelector('[data-act="response"]')?.classList.remove("active"),this.loadDoc()}}),this.responsePanel.open())}showTooltip(e,n){this.hideTooltip();let t=document.createElement("div");t.className="pen-tooltip",t.setAttribute("data-pen-ui",""),t.textContent=n,this.layer.appendChild(t);let o=e.getBoundingClientRect();t.style.left=`${window.scrollX+o.right+8}px`,t.style.top=`${window.scrollY+o.top}px`,this.tooltip=t}hideTooltip(){this.tooltip?.remove(),this.tooltip=void 0}renderLogin(){this.loginEl?.remove();let e=document.createElement("div");if(e.className="pen-login",e.setAttribute("data-pen-ui",""),this.user)e.innerHTML=`<span class="pen-title">Signed in as <span class="pen-name">${g(this.user.name??"you")}</span>${this.isAuthor?' <span class="pen-badge">author</span>':""}</span>
        ...
        <div class="pen-providers"><input type="email" placeholder="you@email.com">
          <button class="pen-btn" data-act="email">Email me a link</button></div>`;let n=async()=>{let t=e.querySelector("input"),o=t.value.trim();if(!o)return;let i=await this.api.emailLogin(o);i.link?location.href=i.link:(t.value="",t.placeholder="Check your email \u2709\uFE0F")};e.querySelector('[data-act="email"]').addEventListener("click",n),e.querySelector("input").addEventListener("keydown",t=>{t.key==="Enter"&&n()})}document.body.appendChild(e),this.loginEl=e}flashLogin(){this.loginEl?.animate([{transform:"scale(1)"},{transform:"scale(1.06)"},{transform:"scale(1)"}],{duration:380,iterations:2})}},g=s=>String(s).replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e]);function I(){let s=window.PENUMBRA;if(!s?.api){console.warn("[penumbra] window.PENUMBRA.api is not set; annotator disabled.");return}let e=new y(s);window.penumbra=e,e.init().catch(n=>console.error("[penumbra] init failed",n)),document.addEventListener("nav",()=>e.reload().catch(n=>console.error("[penumbra] reload failed",n)))}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",I):I();})();
