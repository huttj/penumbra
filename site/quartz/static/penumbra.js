...
`)+`
`}function y(s){let e=s.trim();return e?e.replace(/[\p{Extended_Pictographic}\u{1F1E6}-\u{1F1FF}\u{1F3FB}-\u{1F3FF}️‍\s]/gu,"")===""&&/\p{Extended_Pictographic}/u.test(e):!1}function h(s){return L(s).replace(/!\[([^\]]*)\]\(([^)\s]+)\)/g,'<img alt="$1" src="$2">').replace(/\[([^\]]+)\]\(([^)\s]+)\)/g,'<a href="$2" target="_blank" rel="noopener">$1</a>').replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/(^|[^*])\*([^*\n]+)\*/g,"$1<em>$2</em>")}function l(s){let e=(s??"").replace(/\r\n/g,`
`).split(`
`),n=[],t=0;for(;t<e.length;){let o=e[t];if(/^```/.test(o)){let i=[];for(t++;t<e.length&&!/^```/.test(e[t]);)i.push(e[t++]);t++,n.push(`<pre><code>${L(i.join(`
`))}</code></pre>`);continue}if(/^\s*$/.test(o)){t++;continue}if(/^#{1,6}\s/.test(o)){let i=/^(#{1,6})\s+(.*)$/.exec(o);n.push(`<h${i[1].length}>${h(i[2])}</h${i[1].length}>`),t++;continue}if(/^\s*([-*_])\1{2,}\s*$/.test(o)){n.push("<hr>"),t++;continue}if(/^\s*>/.test(o)){let i=[];for(;t<e.length&&/^\s*>/.test(e[t]);)i.push(e[t++].replace(/^\s*>\s?/,""));n.push(`<blockquote>${l(i.join(`
...
`;async function Q(){let s=window;return s.__PenumbraResponsePanel||await new Promise((e,n)=>{let t=document.querySelector('script[src*="penumbra.js"]'),o=t?t.src.replace(/penumbra\.js(\?.*)?$/,"penumbra-editor.js"):"/static/penumbra-editor.js",r=document.createElement("script");r.src=o,r.onload=()=>e(),r.onerror=()=>n(new Error("editor failed to load")),document.head.appendChild(r)}),s.__PenumbraResponsePanel}var z=typeof globalThis.Highlight<"u"&&!!window.CSS?.highlights,k=10,O=["\u{1F44D}","\u2764\uFE0F","\u{1F525}","\u{1F604}","\u{1F914}","\u{1F3AF}"],g=class{constructor(e){this.user=null;this.isAuthor=!1;this.preamble="";this.blocks=[];this.highlightsOn=!0;this.focused=null;this.hovered=null;this.hoverRaf=!1;this.relayoutQueued=!1;this.railEntries=[];this.repoQueued=!1;this.railLeft=0;this.quietTimer=null;this.blockById=e=>this.blocks.find(n=>n.id===e);this.docY=e=>e.getBoundingClientRect().top+window.scrollY;this.cards=()=>this.blocks.filter(e=>!e.isEmoji&&e.ranges.length);this.chips=()=>this.blocks.filter(e=>e.isEmoji&&e.ranges.length);this.cfg=e,this.api=new d(e.api),this.root=this.resolveRoot(),this.source=this.computeSource(),this.commitSha=e.commitSha??null}resolveRoot(){return(this.cfg.root?document.querySelector(this.cfg.root):null)??document.body}computeSource(){if(this.cfg.source)return this.cfg.source;if(this.cfg.sourceBase){let e=location.pathname.replace(/\/index\.html?$/i,"/").replace(/\.html?$/i,"").replace(/\/$/,"");return this.cfg.sourceBase.replace(/\/$/,"")+e}return location.href}async init(){this.styleEl=document.createElement("style"),this.styleEl.setAttribute("data-pen",""),this.styleEl.textContent=$,document.head.appendChild(this.styleEl),this.layer=document.createElement("div"),this.layer.setAttribute("data-pen-ui",""),this.layer.style.cssText="position:absolute;top:0;left:0;width:0;height:0;",document.body.appendChild(this.layer),this.api.captureTokenFromHash();let e=await this.api.me();this.user=e.user,this.isAuthor=e.isAuthor,this.renderToolbar(),this.renderLogin(),await this.loadDoc(),document.addEventListener("mouseup",n=>{n.target.closest("[data-pen-ui]")||setTimeout(()=>this.onSelection(),0)}),document.addEventListener("mousedown",n=>this.onDocMouseDown(n)),document.addEventListener("click",n=>this.onDocClick(n)),document.addEventListener("mousemove",n=>this.onMouseMove(n),{passive:!0}),document.addEventListener("keydown",n=>{n.key==="Escape"&&(this.compose?this.dismissCompose():this.focused&&(this.focused=null,this.renderAll()))}),window.addEventListener("resize",()=>this.queueRelayout(),{passive:!0})}async reload(){this.dismissCompose(),this.removeQuoteBtn(),this.focused=this.hovered=null,this.styleEl.isConnected||document.head.appendChild(this.styleEl),this.layer.isConnected||document.body.appendChild(this.layer),this.toolbar?.isConnected||this.renderToolbar(),this.loginEl?.isConnected||this.renderLogin(),this.root=this.resolveRoot(),this.source=this.computeSource(),await this.loadDoc()}async loadDoc(){let e="";this.user&&(e=(await this.api.getResponse(this.source).catch(()=>null))?.body??""),this.parse(e),this.renderAll()}parse(e){let{preamble:n,blocks:t}=q(e);this.preamble=n,this.blocks=t.map((o,r)=>({id:`b${r}`,quotes:o.quotes,note:o.note,isEmoji:y(o.note),ranges:o.quotes.map(i=>c([{type:"TextQuoteSelector",exact:i}],this.root)).filter(Boolean)}))}async saveDoc(){let e=w(this.preamble,this.blocks.map(t=>({quotes:t.quotes,note:t.note}))),n=x(e).map((t,o)=>({id:`q${o}`,text:t,selector:b(t,this.root)??[{type:"TextQuoteSelector",exact:t}]}));try{await this.api.saveResponse(this.source,e,n,this.commitSha)}catch(t){alert("Could not save: "+t.message);return}this.focused=null,this.parse(e),this.renderAll()}saveQuiet(e){let n=t=>{let o=e?.querySelector("[data-cardsave]");o&&(o.textContent=t)};n("saving\u2026"),clearTimeout(this.quietTimer),this.quietTimer=setTimeout(async()=>{let t=w(this.preamble,this.blocks.map(r=>({quotes:r.quotes,note:r.note}))),o=x(t).map((r,i)=>({id:`q${i}`,text:r,selector:b(r,this.root)??[{type:"TextQuoteSelector",exact:r}]}));try{await this.api.saveResponse(this.source,t,o,this.commitSha),n("saved")}catch{n("save failed")}},600)}renderAll(){this.renderHighlights(),this.layoutRightRail(),this.layoutLeftRail()}queueRelayout(){this.relayoutQueued||(this.relayoutQueued=!0,requestAnimationFrame(()=>{this.relayoutQueued=!1,this.renderAll()}))}renderHighlights(){if(!z)return;let e=window.CSS.highlights,n=globalThis.Highlight;if(this.responsePanel)return;if(!this.highlightsOn){e.delete("penumbra-quote"),e.delete("penumbra-quote-active"),e.delete("penumbra-draft");return}let t=this.blocks.flatMap(r=>r.ranges);t.length?e.set("penumbra-quote",new n(...t)):e.delete("penumbra-quote"),this.composeCtx?.range?e.set("penumbra-draft",new n(this.composeCtx.range)):e.delete("penumbra-draft");let o=this.blockById(this.hovered??this.focused)?.ranges[0];o?e.set("penumbra-quote-active",new n(o)):e.delete("penumbra-quote-active")}layoutRightRail(){if(this.railRO?.disconnect(),this.layer.querySelectorAll(".pen-card.rail").forEach(t=>t.remove()),this.railEntries=[],!this.highlightsOn||this.responsePanel)return;let e=this.root.getBoundingClientRect();if(window.innerWidth-e.right<300)return;this.railLeft=window.scrollX+e.right+24;let n=this.cards().sort((t,o)=>this.docY(t.ranges[0])-this.docY(o.ranges[0]));if(n.length){for(let t of n){let o=this.buildCard(t,this.focused===t.id);o.style.left=`${this.railLeft}px`,o.style.top="-9999px",this.layer.appendChild(o),this.railEntries.push({el:o,blk:t})}this.railRO=new ResizeObserver(()=>this.queueReposition()),this.railEntries.forEach(t=>this.railRO.observe(t.el)),this.repositionRail()}}queueReposition(){this.repoQueued||(this.repoQueued=!0,requestAnimationFrame(()=>{this.repoQueued=!1,this.repositionRail()}))}repositionRail(){let e=this.railEntries;if(!e.length)return;let n=e.map(i=>i.el.offsetHeight),t=e.map(i=>i.blk.ranges[0]?this.docY(i.blk.ranges[0]):0),o=t.slice(),r=e.findIndex(i=>i.blk.id===this.focused);if(r>=0){o[r]=t[r];for(let i=r+1;i<e.length;i++)o[i]=Math.max(t[i],o[i-1]+n[i-1]+k);for(let i=r-1;i>=0;i--)o[i]=Math.min(t[i],o[i+1]-n[i]-k)}else for(let i=1;i<e.length;i++)o[i]=Math.max(t[i],o[i-1]+n[i-1]+k);e.forEach((i,a)=>i.el.style.top=`${Math.max(0,o[a])}px`)}layoutLeftRail(){if(this.layer.querySelectorAll(".pen-emote").forEach(o=>o.remove()),!this.highlightsOn||this.responsePanel)return;let e=this.root.getBoundingClientRect(),n=window.scrollX+Math.max(6,e.left-40),t=0;for(let o of this.chips().sort((r,i)=>this.docY(r.ranges[0])-this.docY(i.ranges[0]))){let r=document.createElement("div");r.className="pen-emote",r.setAttribute("data-pen-ui",""),r.dataset.blockId=o.id,r.textContent=o.note.trim(),r.addEventListener("mouseenter",()=>this.setHovered(o.id)),r.addEventListener("mouseleave",()=>this.setHovered(null)),r.addEventListener("click",()=>this.focus(o.id)),this.layer.appendChild(r);let i=Math.max(this.docY(o.ranges[0]),t+6);r.style.left=`${n}px`,r.style.top=`${i}px`,t=i+r.offsetHeight}}buildCard(e,n){let t=document.createElement("div");t.className=`pen-card rail ${n?"focused":"compact"}`,t.setAttribute("data-pen-ui",""),t.dataset.blockId=e.id;let o=e.quotes.map(r=>`<div class="pen-quote">${E(r)}</div>`).join("");if(n){t.innerHTML=`${o}
      ...
      ${this.isAuthor?`<button class="pen-tbtn" data-act="reviews" title="See everyone's responses">\u{1F441} Reviews</button>`:""}`,e.querySelector('[data-act="toggle"]').addEventListener("click",()=>{this.highlightsOn=!this.highlightsOn,e.querySelector('[data-act="toggle"]').classList.toggle("active",this.highlightsOn),this.renderAll()}),e.querySelector('[data-act="response"]').addEventListener("click",()=>this.toggleResponse()),e.querySelector('[data-act="reviews"]')?.addEventListener("click",()=>this.toggleReviews()),document.body.appendChild(e),this.toolbar=e}toggleReviews(){if(this.reviewsPanel){this.reviewsPanel.close();return}this.reviewsPanel=new u({api:this.api,root:this.root,source:this.source,onClose:()=>{this.reviewsPanel=void 0}}),this.reviewsPanel.open()}async toggleResponse(){if(this.responsePanel){this.responsePanel.close();return}if(!this.user)return this.flashLogin();let e;this.toolbar?.querySelector('[data-act="response"]')?.classList.add("active");try{e=await Q()}catch{this.toolbar?.querySelector('[data-act="response"]')?.classList.remove("active"),alert("Could not load the editor.");return}this.responsePanel||(this.layer.querySelectorAll(".pen-card.rail, .pen-emote").forEach(n=>n.remove()),this.responsePanel=new e({api:this.api,root:this.root,source:this.source,commitSha:this.commitSha,userName:this.user.name??"you",onClose:()=>{this.responsePanel=void 0,this.removeQuoteBtn(),this.toolbar?.querySelector('[data-act="response"]')?.classList.remove("active"),this.loadDoc()}}),this.responsePanel.open())}showTooltip(e,n){this.hideTooltip();let t=document.createElement("div");t.className="pen-tooltip",t.setAttribute("data-pen-ui",""),t.textContent=n,this.layer.appendChild(t);let o=e.getBoundingClientRect();t.style.left=`${window.scrollX+o.right+8}px`,t.style.top=`${window.scrollY+o.top}px`,this.tooltip=t}hideTooltip(){this.tooltip?.remove(),this.tooltip=void 0}renderLogin(){this.loginEl?.remove();let e=document.createElement("div");if(e.className="pen-login",e.setAttribute("data-pen-ui",""),this.user)e.innerHTML=`<span class="pen-title">Signed in as <span class="pen-name">${E(this.user.name??"you")}</span>${this.isAuthor?' <span class="pen-badge">author</span>':""}</span>
        ...
        <div class="pen-providers"><input type="email" placeholder="you@email.com">
          <button class="pen-btn" data-act="email">Email me a link</button></div>`;let n=async()=>{let t=e.querySelector("input"),o=t.value.trim();if(!o)return;let r=await this.api.emailLogin(o);r.link?location.href=r.link:(t.value="",t.placeholder="Check your email \u2709\uFE0F")};e.querySelector('[data-act="email"]').addEventListener("click",n),e.querySelector("input").addEventListener("keydown",t=>{t.key==="Enter"&&n()})}document.body.appendChild(e),this.loginEl=e}flashLogin(){this.loginEl?.animate([{transform:"scale(1)"},{transform:"scale(1.06)"},{transform:"scale(1)"}],{duration:380,iterations:2})}},E=s=>String(s).replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e]);function M(s,e=7){let n=()=>{s.style.height="auto";let t=getComputedStyle(s),o=parseFloat(t.lineHeight)||20,r=parseFloat(t.paddingTop)+parseFloat(t.paddingBottom)+parseFloat(t.borderTopWidth)+parseFloat(t.borderBottomWidth),i=o*e+r;s.style.height=`${Math.min(s.scrollHeight,i)}px`,s.style.overflowY=s.scrollHeight>i?"auto":"hidden"};s.addEventListener("input",n),requestAnimationFrame(n)}function P(){let s=window.PENUMBRA;if(!s?.api){console.warn("[penumbra] window.PENUMBRA.api is not set; annotator disabled.");return}let e=new g(s);window.penumbra=e,e.init().catch(n=>console.error("[penumbra] init failed",n)),document.addEventListener("nav",()=>e.reload().catch(n=>console.error("[penumbra] reload failed",n)))}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",P):P();})();
