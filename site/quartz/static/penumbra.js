"use strict";(()=>{var T="penumbra:token",f=class{constructor(e){this.base=e}get token(){return localStorage.getItem(T)}set token(e){e?localStorage.setItem(T,e):localStorage.removeItem(T)}headers(e=!1){let t={};return e&&(t["Content-Type"]="application/json"),this.token&&(t.Authorization=`Bearer ${this.token}`),t}captureTokenFromHash(){let e=/[#&]pen_token=([^&]+)/.exec(location.hash);return e?(this.token=decodeURIComponent(e[1]),history.replaceState(null,"",location.pathname+location.search),!0):!1}async me(){if(!this.token)return{user:null,isAuthor:!1};let e=await fetch(`${this.base}/me`,{headers:this.headers()});if(!e.ok)return{user:null,isAuthor:!1};let t=await e.json();return{user:t.user,isAuthor:!!t.isAuthor}}async list(e,t=[]){let n=new URLSearchParams({source:e});t.length&&n.set("include",t.join(","));let o=await fetch(`${this.base}/annotations?${n}`);return o.ok?(await o.json()).items:[]}async create(e,t,n={}){let o=await fetch(`${this.base}/annotations`,{method:"POST",headers:this.headers(!0),body:JSON.stringify({target:e,kind:n.kind??"comment",body:[{type:"TextualBody",value:t}],docVersion:n.docVersion})});if(!o.ok)throw new Error((await o.json().catch(()=>({}))).error??`create failed (${o.status})`);return o.json()}async reply(e,t){let n=e.split("/annotations/")[1],o=await fetch(`${this.base}/annotations/${n}/replies`,{method:"POST",headers:this.headers(!0),body:JSON.stringify({text:t})});if(!o.ok)throw new Error((await o.json().catch(()=>({}))).error??`reply failed (${o.status})`);return o.json()}async patch(e,t){let n=e.split("/annotations/")[1],o=await fetch(`${this.base}/annotations/${n}`,{method:"PATCH",headers:this.headers(!0),body:JSON.stringify(t)});if(!o.ok)throw new Error(`patch failed (${o.status})`);return o.json()}async remove(e){let t=e.split("/annotations/")[1];await fetch(`${this.base}/annotations/${t}`,{method:"DELETE",headers:this.headers()})}async getResponse(e){let t=await fetch(`${this.base}/responses?source=${encodeURIComponent(e)}`,{headers:this.headers()});return t.ok?(await t.json()).response:null}async saveResponse(e,t,n,o){let s=await fetch(`${this.base}/responses`,{method:"POST",headers:this.headers(!0),body:JSON.stringify({source:e,body:t,quotes:n,sourceSha:o})});if(!s.ok)throw new Error((await s.json().catch(()=>({}))).error??`save failed (${s.status})`);return s.json()}async getAllResponses(e){let t=await fetch(`${this.base}/responses/all?source=${encodeURIComponent(e)}`,{headers:this.headers()});return t.ok?(await t.json()).responses:[]}async submitResponse(e){let t=await fetch(`${this.base}/responses/submit`,{method:"POST",headers:this.headers(!0),body:JSON.stringify({source:e})}),n=await t.json().catch(()=>({}));if(!t.ok)throw new Error(n.error??`submit failed (${t.status})`);return n}loginUrl(e){return`${this.base}/auth/${e}/start?return=${encodeURIComponent(location.href)}`}async emailLogin(e){return(await fetch(`${this.base}/auth/email/start`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,return:location.href})})).json()}async logout(){await fetch(`${this.base}/auth/logout`,{method:"POST",headers:this.headers()}).catch(()=>{}),this.token=null}};function b(i){let e=document.createTreeWalker(i,NodeFilter.SHOW_TEXT,{acceptNode(s){let r=s.parentElement;return r&&r.closest("[data-pen-ui]")?NodeFilter.FILTER_REJECT:NodeFilter.FILTER_ACCEPT}}),t=[],n="",o;for(;o=e.nextNode();){let s=o,r=n.length;n+=s.data,t.push({node:s,start:r,end:n.length})}return{text:n,nodes:t}}function v(i,e){for(let n of i.nodes)if(e>=n.start&&e<=n.end)return{node:n.node,offset:e-n.start};let t=i.nodes[i.nodes.length-1];return t?{node:t.node,offset:t.node.data.length}:null}function A(i,e,t){if(e.nodeType===Node.TEXT_NODE){let s=i.nodes.find(r=>r.node===e);return s?s.start+t:null}let n=e.childNodes[t]??e.childNodes[e.childNodes.length-1];if(!n)return null;let o=i.nodes.find(s=>s.node===n||n.contains(s.node));return o?o.start:null}function x(i,e){let t=b(e),n=A(t,i.startContainer,i.startOffset),o=A(t,i.endContainer,i.endOffset);if(n==null||o==null||o<=n)return null;let s=t.text.slice(n,o),r=t.text.slice(Math.max(0,n-32),n),l=t.text.slice(o,o+32);return[{type:"TextQuoteSelector",exact:s,prefix:r,suffix:l},{type:"TextPositionSelector",start:n,end:o}]}function C(i,e){let t=b(e),n=i.find(d=>d.type==="TextQuoteSelector"),o=i.find(d=>d.type==="TextPositionSelector"),s=-1,r=-1;if(n&&n.exact){let d=H(t.text,n);d>=0&&(s=d,r=d+n.exact.length)}if(s<0&&o&&t.text.slice(o.start,o.end)&&(s=o.start,r=o.end),s<0)return null;let l=v(t,s),a=v(t,r);if(!l||!a)return null;let p=document.createRange();return p.setStart(l.node,l.offset),p.setEnd(a.node,a.offset),p}function L(i,e){let t=i.find(a=>a.type==="TextQuoteSelector");if(!t?.exact)return null;let n=b(e),o=H(n.text,t);if(o<0)return null;let s=v(n,o),r=v(n,o+t.exact.length);if(!s||!r)return null;let l=document.createRange();return l.setStart(s.node,s.offset),l.setEnd(r.node,r.offset),l}function M(i,e){let t=i.trim();if(t.length<8)return null;let n=b(e),o=n.text.indexOf(t);return o<0?null:[{type:"TextQuoteSelector",exact:t,prefix:n.text.slice(Math.max(0,o-32),o),suffix:n.text.slice(o+t.length,o+t.length+32)},{type:"TextPositionSelector",start:o,end:o+t.length}]}function H(i,e){let t=[],n=i.indexOf(e.exact);for(;n>=0;)t.push(n),n=i.indexOf(e.exact,n+1);if(t.length===0)return-1;if(t.length===1)return t[0];let o=t[0],s=-1;for(let r of t){let l=0;if(e.prefix){let a=i.slice(Math.max(0,r-e.prefix.length),r);l+=D(a,e.prefix)}if(e.suffix){let a=i.slice(r+e.exact.length,r+e.exact.length+e.suffix.length);l+=F(a,e.suffix)}l>s&&(s=l,o=r)}return o}var F=(i,e)=>{let t=0;for(;t<i.length&&t<e.length&&i[t]===e[t];)t++;return t},D=(i,e)=>{let t=0;for(;t<i.length&&t<e.length&&i[i.length-1-t]===e[e.length-1-t];)t++;return t};var P=i=>i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");function w(i){return P(i).replace(/!\[([^\]]*)\]\(([^)\s]+)\)/g,'<img alt="$1" src="$2">').replace(/\[([^\]]+)\]\(([^)\s]+)\)/g,'<a href="$2" target="_blank" rel="noopener">$1</a>').replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/(^|[^*])\*([^*\n]+)\*/g,"$1<em>$2</em>")}function y(i){let e=(i??"").replace(/\r\n/g,`
`).split(`
`),t=[],n=0;for(;n<e.length;){let o=e[n];if(/^```/.test(o)){let r=[];for(n++;n<e.length&&!/^```/.test(e[n]);)r.push(e[n++]);n++,t.push(`<pre><code>${P(r.join(`
`))}</code></pre>`);continue}if(/^\s*$/.test(o)){n++;continue}if(/^#{1,6}\s/.test(o)){let r=/^(#{1,6})\s+(.*)$/.exec(o);t.push(`<h${r[1].length}>${w(r[2])}</h${r[1].length}>`),n++;continue}if(/^\s*([-*_])\1{2,}\s*$/.test(o)){t.push("<hr>"),n++;continue}if(/^\s*>/.test(o)){let r=[];for(;n<e.length&&/^\s*>/.test(e[n]);)r.push(e[n++].replace(/^\s*>\s?/,""));t.push(`<blockquote>${y(r.join(`
`))}</blockquote>`);continue}if(/^\s*[-*+]\s/.test(o)){let r=[];for(;n<e.length&&/^\s*[-*+]\s/.test(e[n]);)r.push(`<li>${w(e[n++].replace(/^\s*[-*+]\s+/,""))}</li>`);t.push(`<ul>${r.join("")}</ul>`);continue}if(/^\s*\d+\.\s/.test(o)){let r=[];for(;n<e.length&&/^\s*\d+\.\s/.test(e[n]);)r.push(`<li>${w(e[n++].replace(/^\s*\d+\.\s+/,""))}</li>`);t.push(`<ol>${r.join("")}</ol>`);continue}let s=[];for(;n<e.length&&!/^\s*$/.test(e[n])&&!/^(#{1,6}\s|```|\s*>|\s*[-*+]\s|\s*\d+\.\s)/.test(e[n]);)s.push(e[n++]);t.push(`<p>${w(s.join(" "))}</p>`)}return t.join(`
`)}var $=!!window.CSS?.highlights&&typeof globalThis.Highlight<"u",S=class{constructor(e){this.body="";this.quotes=[];this.mode="write";this.seq=0;this.saveTimer=null;this.savedAt="";this.api=e.api,this.root=e.root,this.source=e.source,this.commitSha=e.commitSha,this.userName=e.userName,this.onClose=e.onClose}async open(){let e=await this.api.getResponse(this.source).catch(()=>null);e&&(this.body=e.body??"",this.quotes=e.quotes??[],this.savedAt=e.updated??""),this.build(),this.renderQuoteHighlights()}close(){if(this.flushSave(),$){let e=window.CSS.highlights;e.delete("penumbra-quote"),e.delete("penumbra-quote-active")}this.el?.remove(),O(),this.onClose()}build(){let e=document.createElement("div");e.className="pen-panel",e.setAttribute("data-pen-ui",""),e.innerHTML=`
      ...
      <div class="pen-preview" data-preview hidden></div>`,document.body.appendChild(e),this.el=e,j(e.offsetWidth),this.ta=e.querySelector("[data-essay]"),this.ta.value=this.body,e.querySelector('[data-act="close"]').addEventListener("click",()=>this.close()),e.querySelector('[data-act="mode"]').addEventListener("click",()=>this.toggleMode()),e.querySelector('[data-act="submit"]').addEventListener("click",()=>this.submit()),this.ta.addEventListener("input",()=>{this.body=this.ta.value,this.scheduleSave()}),this.ta.addEventListener("paste",t=>this.onPaste(t)),this.ta.addEventListener("keyup",()=>this.amplifyAtCursor()),this.ta.addEventListener("click",()=>this.amplifyAtCursor())}appendQuote(e){let t=x(e,this.root);if(!t)return;let n=t.find(o=>o.type==="TextQuoteSelector")?.exact??"";n&&(this.quotes.some(o=>o.text===n)||this.quotes.push({id:`q${++this.seq}`,selector:t,text:n}),this.body=`${this.ta.value.replace(/\s+$/,"")}

> ${n.replace(/\n/g," ")}

`,this.ta.value=this.body,this.ta.selectionStart=this.ta.selectionEnd=this.ta.value.length,this.ta.focus(),this.renderQuoteHighlights(),this.scheduleSave())}onPaste(e){let t=e.clipboardData?.getData("text/plain")??"";setTimeout(()=>{this.body=this.ta.value,this.scheduleSave();let n=M(t,this.root);if(n){let o=n.find(s=>s.type==="TextQuoteSelector").exact;this.quotes.some(s=>s.text===o)||(this.quotes.push({id:`q${++this.seq}`,selector:n,text:o}),this.renderQuoteHighlights())}},0)}quoteRange(e){return L(e.selector,this.root)}renderQuoteHighlights(){if(!$)return;let e=this.quotes.map(n=>this.quoteRange(n)).filter(Boolean),t=window.CSS.highlights;e.length?t.set("penumbra-quote",new globalThis.Highlight(...e)):t.delete("penumbra-quote")}amplifyAtCursor(){let e=this.ta.value,t=this.ta.selectionStart,n=e.lastIndexOf(`
`,t-1)+1,o=e.indexOf(`
`,t);o<0&&(o=e.length);let s=e.slice(n,o).replace(/^>\s?/,"").trim(),r=s.length>=6?this.quotes.find(l=>l.text.includes(s)||s.includes(l.text)):void 0;this.amplify(r)}amplify(e){if(!$)return;let t=window.CSS.highlights,n=e?this.quoteRange(e):null;n?t.set("penumbra-quote-active",new globalThis.Highlight(n)):t.delete("penumbra-quote-active")}toggleMode(){this.mode=this.mode==="write"?"preview":"write";let e=this.el.querySelector("[data-preview]"),t=this.el.querySelector('[data-act="mode"]');this.mode==="preview"?(e.innerHTML=y(this.composeMarkdown()),e.querySelectorAll("blockquote").forEach(n=>{let o=(n.textContent??"").trim(),s=this.quotes.find(r=>r.text.includes(o)||o.includes(r.text));s&&(n.addEventListener("mouseenter",()=>this.amplify(s)),n.addEventListener("mouseleave",()=>this.amplify(void 0)))}),e.hidden=!1,this.ta.hidden=!0,t.textContent="Edit"):(e.hidden=!0,this.ta.hidden=!1,t.textContent="Preview")}composeMarkdown(){return this.body.trim()}scheduleSave(){this.setSave("saving\u2026"),clearTimeout(this.saveTimer),this.saveTimer=setTimeout(()=>this.flushSave(),800)}async flushSave(){clearTimeout(this.saveTimer);try{let e=await this.api.saveResponse(this.source,this.body,this.quotes,this.commitSha);this.savedAt=e.updated??"",this.setSave("saved")}catch{this.setSave("save failed")}}setSave(e){let t=this.el?.querySelector("[data-save]");t&&(t.textContent=e)}async submit(){await this.flushSave(),this.setSave("submitting\u2026");try{let e=await this.api.submitResponse(this.source);this.setSave("submitted \u2713"),e.url&&window.open(e.url,"_blank")}catch(e){let t=String(e.message).includes("not configured");this.setSave(t?"submit not enabled yet":"submit failed"),alert(t?"Submitting to the repo isn't enabled yet \u2014 the author needs to add a GitHub token. Your draft is saved.":"Submit failed: "+e.message)}}},k=class{constructor(e){this.reviews=[];this.api=e.api,this.root=e.root,this.source=e.source,this.onClose=e.onClose}async open(){this.reviews=await this.api.getAllResponses(this.source).catch(()=>[]);let e=document.createElement("div");e.className="pen-panel",e.setAttribute("data-pen-ui",""),e.innerHTML=`
      ...
      <div class="pen-reviews" data-list></div>`;let t=e.querySelector("[data-list]");t.innerHTML=this.reviews.length?this.reviews.map((n,o)=>{let s=(n.quotes??[]).filter(r=>!r.dismissed);return`<div class="pen-review">
            <div class="pen-review-head"><span class="pen-name">${N(n.creator?.name??"reader")}</span>
              <span class="pen-savestate">${Y(n.updated)} \xB7 ${n.status}</span></div>
            <div class="pen-md">${y(n.body||"_(quotes only \u2014 no writing yet)_")}</div>
            ${s.length?`<div class="pen-review-quotes">${s.map((r,l)=>`<a class="pen-qchip" data-ri="${o}" data-qi="${l}">\u201C${N(X(r.text))}\u201D</a>`).join("")}</div>`:""}
          </div>`}).join(""):'<p style="padding:14px;color:var(--pen-muted)">No reviews yet.</p>',e.querySelector('[data-act="close"]').addEventListener("click",()=>this.close()),t.querySelectorAll(".pen-qchip").forEach(n=>n.addEventListener("click",()=>this.focusQuote(+n.dataset.ri,+n.dataset.qi))),document.body.appendChild(e),this.el=e,j(e.offsetWidth)}focusQuote(e,t){let n=this.reviews[e]?.quotes?.[t];if(!n)return;let o=L(n.selector,this.root);if(!o)return;let s=window.CSS?.highlights;s&&s.set("penumbra-quote-active",new globalThis.Highlight(o));let r=o.getBoundingClientRect();window.scrollTo({top:window.scrollY+r.top-120,behavior:"smooth"})}close(){let e=window.CSS?.highlights;e&&e.delete("penumbra-quote-active"),this.el?.remove(),O(),this.onClose()}};function N(i){return String(i).replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}var Y=i=>{try{return new Date(i).toLocaleDateString()}catch{return""}},X=(i,e=60)=>i.length>e?i.slice(0,e)+"\u2026":i;function j(i){document.body.classList.add("pen-panel-open"),document.body.style.transition="margin-right .2s ease",document.body.style.marginRight=`${i}px`}function O(){document.body.classList.remove("pen-panel-open"),document.body.style.marginRight=""}var R=`
