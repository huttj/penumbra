...
`).split(`
`),t=0,n=[];for(;t<e.length&&!A(e[t]);)n.push(e[t]),t++;let o=[];for(;t<e.length;){let i=[];for(;t<e.length&&A(e[t]);){let d=Y(e[t]);d.text.trim()&&i.push({text:d.text.trim(),nth:d.nth}),t++}let r=[];for(;t<e.length&&!A(e[t]);)r.push(e[t]),t++;for(;r.length&&r[0].trim()==="";)r.shift();for(;r.length&&r[r.length-1].replace(/​/g,"").trim()==="";)r.pop();let a=[],p=[];i.length&&(a=ae(i.map(d=>d.text).join(" ")),p=a.map((d,h)=>h===0?i[0].nth:1)),o.push({quotes:a,nths:p,note:r.join(`
`)})}return{preamble:n.join(`
`).trim(),blocks:o}}function N(s,e){let t=[];for(let r of e){let a=r.note.replace(/^\n+/,"").replace(/[\s​]+$/,""),p=r.quotes.map(h=>(h??"").replace(/\n/g," ").trim()).filter(Boolean);if(!p.length&&!a.trim())continue;let d=p.length?re(r.nths?.[0]??1,p.join(" ")):"";t.push(d&&a?`${d}
