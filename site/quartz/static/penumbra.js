...
`)),e}scheduleSave(){this.setSave("saving\u2026"),clearTimeout(this.saveTimer),this.saveTimer=setTimeout(()=>this.flushSave(),800)}async flushSave(){clearTimeout(this.saveTimer);try{let e=await this.api.saveResponse(this.source,this.body,this.quotes,this.commitSha);this.savedAt=e.updated??"",this.setSave("saved")}catch{this.setSave("save failed")}}setSave(e){let t=this.el?.querySelector("[data-save]");t&&(t.textContent=e)}async submit(){await this.flushSave(),this.setSave("submitting\u2026");try{let e=await this.api.submitResponse(this.source);this.setSave("submitted \u2713"),e.url&&window.open(e.url,"_blank")}catch(e){let t=String(e.message).includes("not configured");this.setSave(t?"submit not enabled yet":"submit failed"),alert(t?"Submitting to the repo isn't enabled yet \u2014 the author needs to add a GitHub token. Your draft is saved.":"Submit failed: "+e.message)}}},k=class{constructor(e){this.reviews=[];this.api=e.api,this.root=e.root,this.source=e.source,this.onClose=e.onClose}async open(){this.reviews=await this.api.getAllResponses(this.source).catch(()=>[]);let e=document.createElement("div");e.className="pen-panel",e.setAttribute("data-pen-ui",""),e.innerHTML=`
      ...
      <div class="pen-reviews" data-list></div>`;let t=e.querySelector("[data-list]");t.innerHTML=this.reviews.length?this.reviews.map((n,s)=>{let o=(n.quotes??[]).filter(i=>!i.dismissed);return`<div class="pen-review">
            <div class="pen-review-head"><span class="pen-name">${R(n.creator?.name??"reader")}</span>
              <span class="pen-savestate">${B(n.updated)} \xB7 ${n.status}</span></div>
            <div class="pen-md">${y(n.body||"_(quotes only \u2014 no writing yet)_")}</div>
            ${o.length?`<div class="pen-review-quotes">${o.map((i,d)=>`<a class="pen-qchip" data-ri="${s}" data-qi="${d}">\u201C${R(X(i.text))}\u201D</a>`).join("")}</div>`:""}
          </div>`}).join(""):'<p style="padding:14px;color:var(--pen-muted)">No reviews yet.</p>',e.querySelector('[data-act="close"]').addEventListener("click",()=>this.close()),t.querySelectorAll(".pen-qchip").forEach(n=>n.addEventListener("click",()=>this.focusQuote(+n.dataset.ri,+n.dataset.qi))),document.body.appendChild(e),this.el=e,j(e.offsetWidth)}focusQuote(e,t){let n=this.reviews[e]?.quotes?.[t];if(!n)return;let s=L(n.selector,this.root);if(!s)return;let o=window.CSS?.highlights;o&&o.set("penumbra-quote-active",new globalThis.Highlight(s));let i=s.getBoundingClientRect();window.scrollTo({top:window.scrollY+i.top-120,behavior:"smooth"})}close(){let e=window.CSS?.highlights;e&&e.delete("penumbra-quote-active"),this.el?.remove(),I(),this.onClose()}};function R(r){return String(r).replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}var B=r=>{try{return new Date(r).toLocaleDateString()}catch{return""}},X=(r,e=60)=>r.length>e?r.slice(0,e)+"\u2026":r;function j(r){document.body.style.transition="margin-right .2s ease",document.body.style.marginRight=`${r}px`}function I(){document.body.style.marginRight=""}var q=`
